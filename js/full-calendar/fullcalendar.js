...
function ResourceEventRenderer() {
        ...
        for (i=0; i<segCnt; i++) {
            ...
            if (eventElement = seg.element) {
                ...
                if (seg.contentTop !== undefined && height - seg.contentTop < 10) {
                    ...
                    .text(event.title);//MODIFICADO: eliminada la hora del título del evento (día)
    ...
    function slotSegHtml(event, seg) {
        ...
        "<div class='fc-event-head recurso"+event.resourceId+" fc-event-skin'" + skinCssAttr + ">" + //recurso"+event.resourceId+" AÑADIDO POR MI
...
function AgendaView(element, calendar, viewName) {
	...
	function buildSkeleton() {
		...
		for (i=0; i<colCnt; i++) {
			s +=
				"<th class='fc- fc-col" + i + ' ' + headerClass + "'/>"; // fc- needed for setDayID
				//alert("<th class='fc- fc-col" + i + ' ' + headerClass + "'/>");
		}
		/*s+="<th class='fc-agenda-gutter " + headerClass + "'>&nbsp;</th>"+"</tr><tr>"+"<th class='fc-agenda-axis " + headerClass + "'>&nbsp;</th>";//AÑADIDO POR MI
		for (i=0; i<colCnt; i++) {
			s +=
				"<th class='"+ headerClass + "'>MartaTomás</th>"; // fc- needed for setDayID
				//alert("<th class='fc- fc-col" + i + ' ' + headerClass + "'/>");
		}//FIN AÑADIDO POR MI*/
		s +=
			"<th class='fc-agenda-gutter " + headerClass + "'>&nbsp;</th>" +
...
function AgendaEventRenderer() {
		...
		for (i=0; i<segCnt; i++) {
			...
			if (eventElement = seg.element) {
				...
				if (seg.contentTop !== undefined && height - seg.contentTop < 10) {
					...
					eventElement.find('div.fc-event-time')
						//.text(formatDate(event.start, opt('timeFormat')) + ' - ' + event.title);
						.text(event.title);//MODIFICADO: eliminada la hora del título del evento (mes)
					eventElement.find('div.fc-event-title')
						.remove();
				}
				trigger('eventAfterRender', event, event, eventElement);
	...
	function slotSegHtml(event, seg) {
		...
		html +=
			...
			"<div class='fc-event-time recurso"+event.resourceId+"'>" + //AÑADIDO POR MI
			htmlEscape(formatDates(event.start, event.end, opt('timeFormat'))) +
			"</div>" +
			"</div>" +
			"<div class='fc-event-content'>" +
...
/*
	Función que pone una línea en la hora actual de los calendarios.
	Ejemplo de llamada (dentro de los parámetros de inicialización del calendario):
	viewRender: function (view,element){
      setTimeline(view,'#calendario');
    }

    IMPORTANTE: para que la función dibuje la línea correctamente, deben definirse los atributos minTime y maxTime
    del FullCalendar como cadenas.
*/

function setTimeline(view,identificador) {
    ...
    var curCalView = jQuery(identificador).fullCalendar('getView');
...
/*
	Función de apoyo para setTimeline().
	Convierte una cadena de hora en minutos
*/
