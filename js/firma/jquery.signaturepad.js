...
function SignaturePad (selector, options) {
  ...
  function drawLine (e, newYOffset) {
    ...
    //MODIFICACIÓN PARA PERMITIR USO DE FIRMA AL HACER ZOOM
    if (/Android/i.test(navigator.userAgent) && !(/Firefox|SamsungBrowser/i.test(navigator.userAgent))){
        offset = getTotalOffset(canvasContext.canvas);//Nueva función definida más abajo
    } 
    else{ 
        offset=$(e.target).offset();//Línea original
    }
    //FIN MODIFICACIÓN PARA PERMITIR USO DE FIRMA AL HACER ZOOM
...
$.fn.signaturePad.defaults = {
  ...
  , errorMessageDraw : 'Por favor, firme el consentimiento' // The error message displayed when drawOnly and no signature is drawn
